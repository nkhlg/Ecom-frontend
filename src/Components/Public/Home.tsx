...
export default function PermanentDrawerLeft() {
  ...
  function sort(s: any) {
    setOrd(s);
    viewSortedProduct(min, max, s).then((data) => {
      setRow(data.data);
    });
  }
    ...
    <Box sx={{ display: "flex" }}>
      ...
      <AppBar sx={{ marginTop: "64px", background: "white" }}>
        <Toolbar>
          <Box sx={{ marginLeft: "200px", color: "#000000" }}>
            Sort by Price:
            <Button style={{ color: "black" }} onClick={() => sort("asc")}>
              Low to high
            </Button>
            <Button style={{ color: "black" }} onClick={() => sort("desc")}>
              High to low
            </Button>
          </Box>
        </Toolbar>
      ...
      <Grid container spacing={2} sx={{ marginTop: "100px" }}>
        {row.map((row) => (
          <Grid item xs={6}>
            <Card
              sx={{ width: "350px", marginTop: "100px", marginLeft: "25px" }}
            >
              <CardMedia sx={{ height: 140 }} image={row.image} />
              <CardContent>
